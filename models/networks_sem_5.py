...
class NGP(nn.Module):
    def __init__(self, scale, rgb_act='Sigmoid', use_skybox=False, embed_a=False, embed_a_len=12, classes=7):
        ...
        rgb_input_dim = self.rgb_encoder.n_output_dims + self.dir_encoder.n_output_dims + 3 + 1
    ...
    def forward(self, x, d, **kwargs):
        ...
        grads = grads.detach()
        # grads = torch.clamp(grads, min=-1e6, max=1e6)
        # if kwargs.get('distill_normal', False):
        #     normals = self.normal_net(h.detach())
        # else:
        #     normals=None
        # sigmas, feat_rgb = self.density(x, return_feat=True)
        ...
        # if torch.any(torch.isnan(refdir)):
        #     print('refdir contains nan')
        # if torch.any(torch.isinf(refdir)):
        #     print('refdir contains inf')
        # # d_enc = self.dir_encoder(refdir, roughness)
        d_enc = self.dir_encoder((refdir+1)/2)
        dotprod = torch.sum(-normals_pred*d, dim=-1, keepdim=True)

        if self.embed_a:
            rgbs = self.rgb_net(torch.cat([x, d_enc, dotprod, feat_rgb, kwargs['embedding_a']], 1))
        else:
            rgbs = self.rgb_net(torch.cat([x, d_enc, dotprod, feat_rgb], 1))
