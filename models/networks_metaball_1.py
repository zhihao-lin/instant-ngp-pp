...
class NGP(nn.Module):
    ...
    def forward_mb(self, x_2d):
        x_2d = (x_2d+self.scale)/(2*self.scale)
        h = self.mb_encoder(x_2d)
        h = self.mb_net(h)
        height_mb = self.mb_act(h[:, 0])
        density_mb = self.mb_act(h[:, 1])
        return height_mb, density_mb
    ...
    def forward_proxy(self, x, d, **kwargs):
        ...
        Outputs:
            ...
            rgbs: (N, 3)
        """
        x_ = (x-self.xyz_min)/(self.xyz_max-self.xyz_min)
        with torch.no_grad():
            h = self.xyz_encoder(x_)
            h = self.xyz_net(h)
            sigmas = self.sigma_act(h[:, 0])
        with torch.no_grad():
            feat_rgb = self.rgb_encoder(x_)
    ...
    def forward(self, x, d, x_2d, **kwargs):
        """
        Inputs:
            x: (N, 3) xyz in [-scale, scale]
            d: (N, 3) directions
        ...
        height_mb, density_mb = self.forward_mb(x_2d)

        return sigmas, rgbs, normals_pred, semantic, height_mb, density_mb
