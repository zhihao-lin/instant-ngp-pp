...
from .rendering_metaball import NEAR_DISTANCE, center_density
...
class NGP(nn.Module):
    def __init__(self, scale, rgb_act='Sigmoid', use_skybox=False, embed_a=False, embed_a_len=12, classes=7):
        ...
        # constants
        L_mb = 16; F_mb = 2; log2_T_mb = 21; N_min_mb = 16
    ...
    def density(self, x, height, return_feat=False):
        """
        Inputs:
        ...
        height = (height+self.scale) / 2
        x_cond = torch.cat([x, height], dim=-1)
        h = self.mb_encoder(x_cond)
        h = self.mb_net(h)
        sigmas_mb = self.mb_act(h[:, 0])
        
        if return_feat: 
            with torch.no_grad():
                feat_rgb = self.rgb_encoder(x)
            return sigmas, sigmas_mb, feat_rgb
        
        return sigmas+sigmas_mb
    
    @torch.no_grad()
    def proxy_density(self, x):
        x = (x-self.xyz_min)/(self.xyz_max-self.xyz_min)
        h = self.xyz_encoder(x)
        h = self.xyz_net(h)
        sigmas = self.sigma_act(h[:, 0])
    ...
    def forward_mb(self, x, height):
        x = (x-self.xyz_min)/(self.xyz_max-self.xyz_min)
        height = (height+self.scale) / 2
        x_cond = torch.cat([x, height], dim=-1)
        h = self.mb_encoder(x_cond)
        h = self.mb_net(h)
        sigmas = self.mb_act(h[:, 0])
        return sigmas, torch.ones((sigmas.shape[0], 3)).cuda()
    
    def forward_proxy(self, x, d, **kwargs):
        """
        Inputs:
            x: (N, 3) xyz in [-scale, scale]
            d: (N, 3) directions
        ...
        if self.embed_a:
            rgbs = self.rgb_net(torch.cat([x, d, feat_rgb, kwargs['embedding_a']], 1))
        else:
    ...
    def forward(self, x, d, height, **kwargs):
        """
        Inputs:
            x: (N, 3) xyz in [-scale, scale]
            d: (N, 3) directions
        ...
        Outputs:
            ...
            rgbs: (N, 3)
        """
        sigmas, sigmas_mb, feat_rgb = self.density(x, height, return_feat=True)
        ...
        rgbs_mb = torch.ones_like(rgbs)
        
        mask = sigmas_mb > center_density/2
        sigmas_ = torch.zeros_like(sigmas).cuda()
        sigmas_[mask] = sigmas_mb[mask]
        sigmas_[~mask] = sigmas[~mask]
        rgbs_ = torch.zeros_like(rgbs).cuda()
        rgbs_[mask] = rgbs_mb[mask]
        rgbs_[~mask] = rgbs[~mask]
        
        # rgbs = (sigmas.unsqueeze(-1).repeat(1,3)*rgbs+sigmas_mb.unsqueeze(-1).repeat(1,3)*rgbs_mb)/(sigmas.unsqueeze(-1).repeat(1,3)+sigmas_mb.unsqueeze(-1).repeat(1,3)+1e-6)
        # sigmas = sigmas+sigmas_mb

        return sigmas_, rgbs_, normals_pred, semantic
    ...
    def uniform_mb_sample(self, resolution=128):
        half_grid_size = self.scale / resolution
        samples = torch.stack(torch.meshgrid(
                torch.linspace(0, 1-half_grid_size, resolution),
                torch.linspace(0, 1-half_grid_size, resolution),
        ...
        dense_xyz = self.xyz_min * (1-samples) + self.xyz_max * samples
        dense_xyz += half_grid_size*torch.rand_like(dense_xyz).cuda()
        density, _ = self.forward_mb(dense_xyz.view(-1,3))
        return density