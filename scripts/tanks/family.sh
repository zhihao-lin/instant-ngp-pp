# train ngp
# python train.py --exp_name family-no \
#     --root_dir ../datasets/TanksAndTempleBG/Family --dataset_name tnt \
#     --batch_size 2048 --scale 8.0 --num_epochs 70 --render_path --render_train --render_rgb \
#     --render_depth --embed_a --embed_a_len 6 --use_skybox \
#     --ckpt_load last.ckpt --val_only 

# gan+ngp: smog
# python train.py --exp_name family-smog --climate smog \
#     --root_dir ../datasets/TanksAndTempleBG/Family --dataset_name tnt \
#     --batch_size 2048 --scale 8.0 --num_epochs 70 --render_path --render_train --render_rgb \
#     --render_depth --embed_a --embed_a_len 6 --use_skybox \
#     --ckpt_load last.ckpt --val_only 

# gan+ngp: flood
# python train.py --exp_name family-flood --climate flood \
#     --root_dir ../datasets/TanksAndTempleBG/Family --dataset_name tnt \
#     --batch_size 2048 --scale 8.0 --num_epochs 70 --render_path --render_train --render_rgb \
#     --render_depth --embed_a --embed_a_len 6 --use_skybox \
#     --ckpt_load last.ckpt --val_only 


# Testing view
# python render.py --config configs/Family.txt --exp_name family-no \
#     --root_dir ../datasets/TanksAndTempleBG/Family --dataset_name tnt \
#     --batch_size 1024 --scale 8.0 --num_epochs 60 --render_path --render_rgb  --render_depth \
#     --embed_a --embed_a_len 8 --use_skybox \
#     --ckpt_load ckpts/tnt/family-no/epoch=149_slim.ckpt 
   

# Smog
# python render.py --config configs/Family.txt --exp_name family-smog-sparse \
#     --root_dir ../datasets/TanksAndTempleBG/Family --dataset_name tnt \
#     --batch_size 1024 --scale 8.0 --num_epochs 60 --render_path --render_rgb \
#     --embed_a --embed_a_len 8 --use_skybox \
#     --ckpt_load ckpts/tnt/family-no/epoch=149_slim.ckpt \
#     --depth_path results/tnt/family-no/depth_raw.npy \
#     --simulate smog --depth_bound 0.9 --sigma 0.5 --rgb 0.925 0.906 0.758 


# Render Panoranma
# python render_panorama.py --config configs/Family.txt --exp_name family-no \
#     --root_dir ../datasets/TanksAndTempleBG/Family --dataset_name tnt \
#     --pano_hw 512 1024 --pano_radius 0.2 \
#     --batch_size 1024 --use_skybox \
#     --ckpt_load ckpts/tnt/family-no/epoch=149_slim.ckpt 

# Flood
# wave param: plane_len=1.0 , ampl_const= 2e6, freq=3.0
python render.py --config configs/Family.txt --exp_name family-flood \
    --root_dir ../datasets/TanksAndTempleBG/Family --dataset_name tnt  \
    --batch_size 1024 --use_skybox --render_path --render_rgb  \
    --ckpt_load ckpts/tnt/family-no/epoch=149_slim.ckpt \
    --simulate water --water_height -0.02 --rgb 0.488 0.406 0.32 --refraction_idx 1.35 --gf_r 5 --gf_eps 0.1 \
    --plane_path ../datasets/TanksAndTempleBG/Family/plane.npy \
    --gl_theta 0.008 --gl_sharpness 500 --wave_len 1.0 --wave_ampl 2000000 \
    --anti_aliasing_factor 1 --chunk_size 600000

# gan+ngp: smog
# python train.py --config configs/Family.txt --exp_name family-smog-gan-ngp \
#     --root_dir ../datasets/TanksAndTempleBG/Family --dataset_name tnt \
#     --batch_size 1024 --scale 8.0 --num_epochs 25 --render_path --render_rgb \
#     --use_skybox --climate smog \
#     --ckpt_load ckpts/tnt/family-smog-gan-ngp/last.ckpt